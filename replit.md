...
## Recent Changes
- **April 27, 2026**: Added image conversion failure tracking. New `image_conversion_failures` JSONB column on `blog_posts` (idempotent `ADD COLUMN IF NOT EXISTS` migration). Backfill now uses `process_image_url_tracked()` which returns `(url, error_or_None)` instead of silently falling back. Per-image failures (URL, type, error message, timestamp) are written to the column on each attempt and cleared on success. `GET /admin/image-status` now returns `total_failed_images` count and `articles_with_failures` list with full failure details.
- **April 27, 2026**: Fixed image pipeline to convert ALL external blog images (any CDN) to WebP, not just `cdn.outrank.so`. Root cause: Outrank changed their CDN to `cdnimg.co`; ~50 articles published after the switch kept raw `.jpg` featured images and inline content images. Fix: (1) `EXTERNAL_IMAGE_PATTERN` in `image_processor.py` now matches any `https?://...jpg|jpeg|png|gif` URL; (2) `process_featured_image()` now converts any external non-WebP URL, not just `cdn.outrank.so`; (3) startup background task `_run_image_backfill()` runs on every restart, finds published articles with unconverted external images, converts them to WebP in object storage, and updates both `markdown_body`/`html_body` and `featured_image` in the DB. Backfill is async (non-blocking), idempotent (only acts on unconverted images), commits per-article so progress survives restarts.
- **April 7, 2026**: Published maternity leave article ("How to write maternity break (without underselling yourself)") at `/blog/articles/maternity-leave-on-resume`. Added `subtitle`, `author_byline`, `image_alt` fields to Outrank webhook schema. Article H1 now uses `post.subtitle or post.title` in article template. Added startup migration to fix metadata for manually-published articles.
- **April 2, 2026**: Comprehensive blog SEO optimization applied to all articles. Template changes (auto-apply to every article): enriched OG tags (`og:image:alt`, `og:image:width/height`, `og:locale`), Twitter tags (`twitter:site @story_cv_`, `twitter:creator`, `twitter:image:alt`), `article:tag` per-tag meta, `title` attribute on featured image. Enriched Article JSON-LD (image as ImageObject 1200×675, author as Person with URL, publisher as Organization with logo, `wordCount`, `keywords`, `articleSection`, `inLanguage`), BreadcrumbList JSON-LD on every article. Global WebSite + Organization schemas added to `templates/base.html`. FAQPage JSON-LD rendered conditionally when `post.faq_items` is present. Added `faq_items` JSONB column to `blog_posts` (safe `ADD COLUMN IF NOT EXISTS` migration runs at startup). Added `extract_faq_items()` utility in `src/utils.py` that parses bold-question and `###` heading FAQ formats. Outrank webhook now auto-extracts and stores FAQ items on every article ingest. Startup backfill populates `faq_items` for existing posts (backfilled 1 of 4 existing posts).
- **January 28, 2026**: Created dedicated pricing page (`/pricing`) with SEO meta tags, Product schema structured data, H1 "Hire a world-class resume writer, for the price of a template", two-tier pricing cards with founding user price animation ($59→$39), Famewall testimonial embed, and CTA section. Created sitemap.xml with all site pages. Testimonial styles scoped with `.pricing-page` parent class to avoid collisions.
- **January 21, 2026**: Added Product Hunt launch banner to all pages (via header.js) with brand color #ee4e34, rocket emoji, "Support Us!" button linking to Product Hunt page, and dismissible via localStorage. Added Product Hunt embed widget to blog article template (with clear comments for easy removal after launch campaign). Removed " | StoryCV Blog" suffix from blog article page titles.
- **January 7, 2026**: Created modular base HTML template (`templates/base.html`) with Jinja2 template inheritance. Blog templates now extend the base template for consistent favicon, fonts, stylesheets, and analytics. Standardized favicon links across all static HTML pages to use `/favicon.svg` with `/favicon.ico` fallback.
