...
## Recent Changes
- **January 5, 2026**: Added storage image serving API endpoint (`GET /api/storage/{path}`) to serve images from Replit Object Storage bucket. Updated image processor to return API-based URLs (`/api/storage/blog-images/{slug}/{filename}.webp`) instead of direct GCS URLs. Images are served with proper content-type headers and 1-year cache control. Added migration script (`scripts/migrate_image_urls.py`) to convert any existing GCS URLs in blog posts to the new API format.
